...
export default function Header() {
  ...
  // Nav link text color
  const getTextClass = () => {
    if (isScrolled) return 'text-white/90 hover:text-[#D4A843] active:text-[#B89020]';
    if (isLightPage) return 'text-[#0F0F0F] hover:text-[#D4A843] active:text-[#B89020]';
    return 'text-white/90 hover:text-[#D4A843] active:text-[#B89020] drop-shadow-[0_1px_4px_rgba(0,0,0,0.8)]';
  };
  ...
  const getBookClass = () => {
    if (isScrolled || !isLightPage) {
      return 'bg-[#D4A843] hover:bg-[#B89020] text-[#0F0F0F] shadow-xl shadow-black/20 hover:shadow-[#D4A843]/20';
    }
    return 'bg-[#0F0F0F] hover:bg-[#D4A843] text-[#D4A843] hover:text-[#0F0F0F] border border-transparent';
  };
                    ...
                    >
                      {dropdownDestinations.map((destination, idx) => (
                        <Link
                          ...
                          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs md:text-sm font-medium text-white/80 hover:text-[#D4A843] hover:bg-white/[0.03] transition-all duration-150 capitalize"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-[#D4A843]/60 shrink-0"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="12" r="3"/></svg>
                          {destination.label}
                        </Link>
                      ))}
                    </motion.div>
        ...
        <div className="fixed inset-0 z-[10001] lg:hidden">
          ...
          <div className="absolute top-0 right-0 h-full w-[280px] bg-[#141414] p-6 flex flex-col justify-between text-white border-l border-white/5">
            ...
            <nav className="flex flex-col gap-6">
              {navLinks.map((link, idx) => (
                <Link
                  ...
                  className="text-base font-semibold uppercase tracking-wide text-white/80 hover:text-[#D4A843] transition-colors"
                >
                  {link.label}
                </Link>
              ))}
              ...
              <div className="border-t border-white/10 pt-4 mt-2">
                <p className="text-[11px] text-[#D4A843] uppercase tracking-wider mb-3">Destinations</p>
              ...
              >
                ...
                Favorites
                {favoritesCount > 0 && (
                  <span className="text-[11px] font-medium px-1.5 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(212,168,67,0.2)', color: '#D4A843' }}>
                    {favoritesCount}
                  </span>
                )}
              </button>
            ...
            <button
              ...
              className="w-full h-11 bg-[#D4A843] text-[#0F0F0F] font-bold rounded-full text-xs uppercase transition-colors hover:bg-[#B89020]"
            >
              Book Now
            </button>
          </div>
