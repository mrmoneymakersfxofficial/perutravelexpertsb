    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            <div className="rounded-2xl overflow-hidden shadow-2xl" style={{ background: 'rgba(255,255,255,0.98)', border: '1px solid rgba(212,168,67,0.2)' }}>
              ...
              <div className="h-px bg-[#E8CC6A]/20" />
                  ...
                  <div className="px-4 py-3">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(locale === 'es' ? popularSearchesEs : popularSearchesEn).map((term) => (
                        <button key={term} onClick={() => handlePopularClick(term)} className="text-[13px] px-3 py-1.5 rounded-full transition-colors" style={{ backgroundColor: '#F8F6F2', color: '#1C1C1C', border: '1px solid rgba(212,168,67,0.15)' }}>
                          {term}
                        </button>
                      ))}
                    </div>
                ...
                {results.map((tour, index) => (
                  <motion.button key={tour.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.025 }} onClick={() => handleSelect(tour)} className="flex items-center gap-3 w-full px-4 py-2.5 hover:bg-[#F8F6F2]/80 transition-colors text-left">
                    ...
                    <div className="text-right shrink-0">
                      <p className="font-bold text-[13px] leading-tight" style={{ color: '#D4A843' }}>${tour.priceUSD}</p>
