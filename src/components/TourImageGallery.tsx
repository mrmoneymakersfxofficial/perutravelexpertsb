...
export default function TourImageGallery({ images = [], title = '' }: TourImageGalleryProps) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      <h3 className="text-lg sm:text-xl font-bold text-[#D6B37F] tracking-wide uppercase mb-5 sm:mb-6 flex items-center gap-2">
        <svg
          className="w-5 h-5"
          fill="none"
          stroke="currentColor"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
        {images.map((img, idx) => (
          <div
            ...
            className={`group relative bg-black rounded-xl overflow-hidden cursor-pointer border border-white/5 transition-all duration-300 hover:border-[#D6B37F]/20 hover:shadow-lg ${
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="relative w-full max-w-5xl mx-auto flex items-center justify-center" style={{ height: '60vh' }}>
              ...
              <button
                onClick={handlePrev}
                className="absolute left-2 sm:left-4 z-10 p-2.5 sm:p-3 rounded-full bg-white/5 hover:bg-[#D6B37F] text-white hover:text-[#0F0F0F] border border-white/10 hover:border-transparent transition-all"
                aria-label="Previous"
              >
              ...
              <button
                onClick={handleNext}
                className="absolute right-2 sm:right-4 z-10 p-2.5 sm:p-3 rounded-full bg-white/5 hover:bg-[#D6B37F] text-white hover:text-[#0F0F0F] border border-white/10 hover:border-transparent transition-all"
                aria-label="Next"
              >
