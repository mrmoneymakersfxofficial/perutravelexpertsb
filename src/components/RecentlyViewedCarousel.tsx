        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {recentTours.length > 3 && (
            <div className="hidden sm:flex items-center gap-2">
              <button onClick={() => scroll('left')} className="w-9 h-9 rounded-full border border-[#E8D5B5]/30 flex items-center justify-center hover:bg-white/80 transition-colors" style={{ color: '#D6B37F' }}><ChevronLeft className="w-4 h-4" /></button>
              <button onClick={() => scroll('right')} className="w-9 h-9 rounded-full border border-[#E8D5B5]/30 flex items-center justify-center hover:bg-white/80 transition-colors" style={{ color: '#D6B37F' }}><ChevronRight className="w-4 h-4" /></button>
            </div>
          )}
        </div>
            ...
            <motion.div key={tour.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className="snap-start shrink-0 w-[240px] sm:w-[260px]">
              <button onClick={() => onTourClick?.(tour)} className="w-full group rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-[#E8D5B5]/10 text-left" style={{ backgroundColor: '#F8F6F2' }}>
                ...
                <div className="p-3">
                  ...
                  <p className="text-sm font-bold mt-1" style={{ color: '#D6B37F' }}>${tour.priceUSD}<span className="text-[10px] font-normal text-[#8B8680] ml-1">{t.tours.perPerson}</span></p>
                </div>
              </button>
            </motion.div>
          ))}
