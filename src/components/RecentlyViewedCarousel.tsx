...
export default function RecentlyViewedCarousel({ onTourClick }: RecentlyViewedCarouselProps) {
  ...
  return (
    <section className="py-8 md:py-12" style={{ backgroundColor: '#0F0F0F' }}>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {recentTours.length > 3 && (
            <div className="hidden sm:flex items-center gap-2">
              <button onClick={() => scroll('left')} className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center hover:bg-white/5 transition-colors" style={{ color: '#D4A843' }}><ChevronLeft className="w-4 h-4" /></button>
              <button onClick={() => scroll('right')} className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center hover:bg-white/5 transition-colors" style={{ color: '#D4A843' }}><ChevronRight className="w-4 h-4" /></button>
            </div>
          )}
        </div>
        <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {recentTours.map((tour, index) => (
            <motion.div key={tour.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className="snap-start shrink-0 w-[240px] sm:w-[260px]">
              <button onClick={() => onTourClick?.(tour)} className="w-full group rounded-xl overflow-hidden transition-all duration-300 border border-white/[0.06] text-left" style={{ backgroundColor: 'rgba(255,255,255,0.03)' }}>
                ...
                <div className="p-3">
                  <h3 className="font-playfair font-semibold text-sm truncate text-white">{locale === 'es' ? tour.nameEs : tour.nameEn}</h3>
                  <p className="text-sm font-bold mt-1" style={{ color: '#D4A843' }}>${tour.priceUSD}<span className="text-[10px] font-normal text-white/40 ml-1">{t.tours.perPerson}</span></p>
                </div>
              </button>
            </motion.div>
          ))}
