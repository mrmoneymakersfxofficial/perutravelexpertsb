...
export default function TourDetailPage({
  ...
  return <TourDetailContent destination={destination} slug={slug} />;
}
      ...
      <section className="relative h-72 sm:h-96 md:h-[500px] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 md:pb-12">
            ...
            <h1 className="font-playfair text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-3">
              {name}
            </h1>
            <div className="flex items-baseline gap-2">
              <span className="text-[#8B8680] text-sm">{t.tours.price}</span>
              <span className="text-3xl md:text-4xl font-bold font-playfair text-[#C8A97E]">
                ${Math.round(tour.priceUSD)}
              </span>
              <span className="text-[#8B8680] text-sm">{t.tours.perPerson}</span>
              {tour.highSeasonPrice && (
                <span className="text-[#8B8680] text-sm ml-2">
              ...
              <div className="sticky top-24 space-y-6">
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E8D5B5]/20">
                  ...
                  <Button
                    ...
                    className="btn-gold rounded-full w-full py-3 text-base"
                  >
                  ...
                  <a
                    ...
                    className="mt-3 flex items-center justify-center gap-2 w-full py-3 rounded-full bg-green-600 text-white font-semibold hover:bg-green-700 transition-colors text-sm"
                  >
                    WhatsApp
                  </a>
                </div>
                ...
                {/* Quick Links */}
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E8D5B5]/20">
                  <h3 className="font-playfair text-lg font-bold mb-3" style={{ color: '#2C1810' }}>
                    {locale === 'es' ? 'Navegación' : 'Navigation'}
                  </h3>
                  <nav className="space-y-2">
                    <Link href={`/tour-packages/${destination}`} className="flex items-center gap-2 text-sm text-[#8B8680] hover:text-[#C8A97E] transition-colors">
                      <ChevronRight className="w-4 h-4" />
                      {t.tourDetail.toursIn} {destName}
                    </Link>
                    <Link href="/tour-packages" className="flex items-center gap-2 text-sm text-[#8B8680] hover:text-[#C8A97E] transition-colors">
                      <ChevronRight className="w-4 h-4" />
                      {t.tourDetail.viewAllTours}
                    </Link>
                    <Link href="/customized-tours" className="flex items-center gap-2 text-sm text-[#8B8680] hover:text-[#C8A97E] transition-colors">
                      <ChevronRight className="w-4 h-4" />
                      {t.customized.title}
                    </Link>
                  </nav>
        ...
        <section className="py-16 md:py-20 border-t border-[#E8D5B5]/20" style={{ backgroundColor: '#FAF8F5' }}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="font-playfair text-3xl md:text-4xl font-bold mb-3" style={{ color: '#2C1810' }}>
