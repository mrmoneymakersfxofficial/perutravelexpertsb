        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <div className="space-y-6 mb-8">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a href={getWhatsAppLink(getDefaultWhatsAppMessage(locale))} target="_blank" rel="noopener noreferrer" className="text-[#8B8680] hover:text-[#D4A843] transition-colors">+51 984 215 157</a>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a href="mailto:info@perutravelexpertsb.com" className="text-[#8B8680] hover:text-[#D4A843] transition-colors">info@perutravelexpertsb.com</a>
                  </div>
                </div>
                <div className="flex items-start gap-4">
              ...
              <a href={getWhatsAppLink(getDefaultWhatsAppMessage(locale))} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-green-600 text-white font-semibold hover:bg-green-700 transition-colors text-sm">
                <MessageCircle className="w-5 h-5" />{t.contact.sendWhatsApp}
              </a>
              <div className="mt-8 rounded-2xl overflow-hidden border border-[#E8CC6A]/[0.06]">
                <div className="w-full h-64 bg-[#E8CC6A]/20 flex items-center justify-center">
                  <div className="text-center">
                    <MapPin className="w-10 h-10 text-[#D4A843] mx-auto mb-2" />
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <form onSubmit={handleWhatsAppSubmit} className="bg-white/[0.02] rounded-2xl p-6 md:p-8 border border-[#E8CC6A]/[0.04] space-y-5">
                <div>
                  <Label htmlFor="contact-name" className="text-sm font-medium" style={{ color: '#1C1C1C' }}>{t.booking.name} *</Label>
                  <Input id="contact-name" name="contact-name" required className="mt-1.5 border-[#E8CC6A]/40 focus:border-[#D4A843]" placeholder={t.booking.name} />
                </div>
                <div>
                  <Label htmlFor="contact-email" className="text-sm font-medium" style={{ color: '#1C1C1C' }}>{t.booking.email} *</Label>
                  <Input id="contact-email" name="contact-email" type="email" required className="mt-1.5 border-[#E8CC6A]/40 focus:border-[#D4A843]" placeholder={t.booking.email} />
                </div>
                <div>
                  <Label htmlFor="contact-subject" className="text-sm font-medium" style={{ color: '#1C1C1C' }}>{locale === 'es' ? 'Asunto' : 'Subject'}</Label>
                  <Input id="contact-subject" name="contact-subject" className="mt-1.5 border-[#E8CC6A]/40 focus:border-[#D4A843]" placeholder={locale === 'es' ? '¿En qué podemos ayudarte?' : 'How can we help you?'} />
                </div>
                <div>
                  <Label htmlFor="contact-message" className="text-sm font-medium" style={{ color: '#1C1C1C' }}>{t.booking.message} *</Label>
                  <Textarea id="contact-message" name="contact-message" required rows={5} className="mt-1.5 border-[#E8CC6A]/40 focus:border-[#D4A843] resize-none" placeholder={locale === 'es' ? 'Cuéntanos más sobre tu viaje ideal...' : 'Tell us more about your ideal trip...'} />
                </div>
