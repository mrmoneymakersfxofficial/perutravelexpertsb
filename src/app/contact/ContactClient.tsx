              ...
              <div className="space-y-6 mb-8">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a href={getWhatsAppLink('Hola, me interesa información sobre los tours de PeruTravelExpertsB')} target="_blank" rel="noopener noreferrer" className="text-[#8B8680] hover:text-[#D6B37F] transition-colors">+51 984 215 157</a>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a href="mailto:info@perutravelexpertsb.com" className="text-[#8B8680] hover:text-[#D6B37F] transition-colors">info@perutravelexpertsb.com</a>
                  </div>
                </div>
                <div className="flex items-start gap-4">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <form onSubmit={handleWhatsAppSubmit} className="bg-white/[0.02] rounded-2xl p-6 md:p-8 border border-[#E8D5B5]/[0.04] space-y-5">
                ...
                <div>
                  ...
                  <Input id="contact-email" name="contact-email" type="email" required className="mt-1.5 border-[#E8D5B5]/40 focus:border-[#D6B37F]" placeholder={t.booking.email} />
                </div>
                <div>
                  <Label htmlFor="contact-subject" className="text-sm font-medium" style={{ color: '#1C1C1C' }}>{locale === 'es' ? 'Asunto' : 'Subject'}</Label>
                  <Input id="contact-subject" name="contact-subject" className="mt-1.5 border-[#E8D5B5]/40 focus:border-[#D6B37F]" placeholder={locale === 'es' ? '¿En qué podemos ayudarte?' : 'How can we help you?'} />
                </div>
                <div>
                  <Label htmlFor="contact-message" className="text-sm font-medium" style={{ color: '#1C1C1C' }}>{t.booking.message} *</Label>
                  <Textarea id="contact-message" name="contact-message" required rows={5} className="mt-1.5 border-[#E8D5B5]/40 focus:border-[#D6B37F] resize-none" placeholder={locale === 'es' ? 'Cuéntanos más sobre tu viaje ideal...' : 'Tell us more about your ideal trip...'} />
                </div>
