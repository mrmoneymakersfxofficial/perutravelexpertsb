...
export default function TourPackagesClient() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#0F0F0F' }}>
      ...
      <section id="packages-destinations" className="py-16 md:py-20" style={{ backgroundColor: '#0F0F0F' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-12">
            <span className="text-xs sm:text-sm text-[#D4A843] font-bold tracking-widest uppercase block mb-2">
                  ...
                  <div className="rounded-2xl overflow-hidden transition-all duration-300 border border-white/[0.06]">
                    ...
                    <div className="p-5 bg-[#141414]">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-white/[0.06]">
                        <span className="text-xs text-[#D4A843]">From ${Math.min(...getToursByDestination(dest.slug).map(t => t.priceUSD))}</span>
                        <span className="flex items-center gap-1 text-sm font-medium text-[#D4A843] group-hover:gap-2 transition-all">{t.tourDetail.viewTours}<ArrowRight className="w-4 h-4" /></span>
                      </div>
                    </div>
                  </div>
                </Link>
      ...
      {/* CTA Section */}
      <section id="packages-cta" className="py-12 md:py-16" style={{ backgroundColor: '#0F0F0F', borderTop: '1px solid rgba(212,168,67,0.1)' }}>
