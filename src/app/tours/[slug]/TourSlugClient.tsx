...
export default function TourSlugClient({ tour }: TourSlugClientProps) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0F0F0F]">
        <div className="text-center px-4">
          ...
          <Link href="/tours">
            <Button className="rounded-full px-6 py-3 text-sm font-semibold uppercase tracking-wide" style={{ backgroundColor: '#D4A843', color: '#0F0F0F' }}>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0F0F0F]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-10 md:gap-12">
            ...
            <div className="lg:col-span-2 space-y-10 sm:space-y-12">
              ...
              <motion.div variants={sectionVariant} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }}>
                ...
                <div className="h-[2px] w-16 mb-4" style={{ background: 'linear-gradient(90deg, #D4A843, #B89020)' }} />
              ...
              <div className="sticky top-24 space-y-6">
                <div className="rounded-2xl p-6 border border-white/[0.08] bg-[#141414]">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-4xl font-bold font-playfair text-[#D4A843]">${Math.round(tour.priceUSD)}</p>
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-md text-[#D4A843] bg-[#D4A843]/10 font-medium">{diffLabel}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                  ...
                  <Button onClick={() => setBookingOpen(true)} className="w-full py-3 rounded-full text-sm font-bold uppercase tracking-wide shadow-lg hover:shadow-xl transition-all duration-200" style={{ backgroundColor: '#D4A843', color: '#0F0F0F' }}>
                ...
                <div className="rounded-2xl p-6 border border-white/[0.08] bg-[#141414]">
                  ...
                  <nav className="space-y-2">
                    <Link href="/tours" className="flex items-center gap-2 text-sm text-white/50 hover:text-[#D4A843] transition-colors"><ChevronRight className="w-4 h-4" />{locale === 'es' ? 'Volver a Tours' : 'Back to Tours'}</Link>
                    <Link href="/tour-packages" className="flex items-center gap-2 text-sm text-white/50 hover:text-[#D4A843] transition-colors"><ChevronRight className="w-4 h-4" />{t.tourDetail.ourDestinations}</Link>
                  </nav>
                </div>
              </div>
            </div>
      ...
      {/* ── Mobile Info Panel ── */}
      <section className="lg:hidden py-6 sm:py-8 bg-[#0A0A0A]" style={{ borderTop: '1px solid rgba(212,168,67,0.1)' }}>
          ...
          <div className="rounded-2xl p-4 sm:p-6 border border-white/[0.08] bg-[#141414]">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold font-playfair text-[#D4A843]">${Math.round(tour.priceUSD)}</p>
                <p className="text-[10px] sm:text-xs text-white/40">{t.tours.perPerson}</p>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
                <span className="text-xs px-2 py-0.5 rounded-md text-[#D4A843] bg-[#D4A843]/10 font-medium">{diffLabel}</span>
                <div className="flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs text-[#D4A843] bg-white/[0.04]"><Clock className="w-3 h-3 sm:w-3.5 sm:h-3.5" />{tour.duration} {t.tours.days}</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4 sm:mb-6">
              <div className="flex items-center gap-2 text-xs sm:text-sm text-white/50"><Mountain className="w-4 h-4 text-[#D4A843]" />{diffLabel}</div>
              <div className="flex items-center gap-2 text-xs sm:text-sm text-white/50"><MapPin className="w-4 h-4 text-[#D4A843]" />{destName}</div>
            </div>
            <div className="flex gap-2 sm:gap-3 mb-4 sm:mb-6">
              ...
              <Button onClick={() => setBookingOpen(true)} className="flex-1 h-11 rounded-full text-xs sm:text-sm font-semibold shadow-lg transition-all duration-200" style={{ backgroundColor: '#D4A843', color: '#0F0F0F' }}>{t.tours.bookNow}</Button>
            </div>
            <nav className="space-y-1.5 sm:space-y-2">
              <Link href="/tours" className="flex items-center gap-2 text-xs sm:text-sm text-white/50 hover:text-[#D4A843] transition-colors"><ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />{locale === 'es' ? 'Volver a Tours' : 'Back to Tours'}</Link>
              <Link href="/tour-packages" className="flex items-center gap-2 text-xs sm:text-sm text-white/50 hover:text-[#D4A843] transition-colors"><ChevronRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />{t.tourDetail.ourDestinations}</Link>
            </nav>
          </div>
        </div>
      </section>
      ...
      <AnimatePresence>
        {!anyModalOpen && (
          <motion.div key="sticky-cta" initial={{ y: '100%', opacity: 0 }} animate={{ y: '0%', opacity: 1 }} exit={{ y: '100%', opacity: 0 }} transition={{ duration: 0.25, ease: [0.4, 0, 0.2, 1] }} className="lg:hidden fixed bottom-0 left-0 right-0 z-[9997] px-3 sm:px-4 py-2.5 sm:py-3" style={{ backgroundColor: 'rgba(15,15,15,0.97)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderTop: '1px solid rgba(212,168,67,0.1)', paddingBottom: 'calc(0.625rem + env(safe-area-inset-bottom))' }}>
            <div className="max-w-7xl mx-auto flex items-center gap-2 sm:gap-3">
              ...
              <Button onClick={() => setBookingOpen(true)} className="flex-1 sm:flex-none sm:px-5 h-10 sm:h-11 rounded-full text-xs sm:text-[13px] font-semibold transition-all duration-200 shadow-lg" style={{ backgroundColor: '#D4A843', color: '#0F0F0F' }}>{t.tours.bookNow}</Button>
      ...
      {/* ── Related Tours ── */}
      {related.length > 0 && (
        <section className="py-12 sm:py-16 md:py-20 bg-[#0F0F0F]" style={{ borderTop: '1px solid rgba(212,168,67,0.1)' }}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-8 sm:mb-12">
              ...
              <div className="w-16 sm:w-20 h-0.5 mx-auto" style={{ background: 'linear-gradient(90deg, #D4A843, #B89020)' }} />
            </div>
                  ...
                  <div className="relative h-48 sm:h-56 overflow-hidden">
                    ...
                    <div className="absolute bottom-3 left-3 right-3">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[#D4A843] font-bold text-xs sm:text-sm">${Math.round(relTour.priceUSD)}</span>
