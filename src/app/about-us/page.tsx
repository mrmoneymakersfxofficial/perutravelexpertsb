...
export default function AboutUsPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#FAF8F5' }}>
      <PageHeader
        title={t.pageHeaders.aboutUs}
        subtitle={t.about.subtitle}
        breadcrumbs={[
          { label: locale === 'es' ? 'Inicio' : 'Home', href: '/' },
          { label: t.pageHeaders.aboutUs },
        ]}
      />
      ...
      <section className="py-12 md:py-16" style={{ backgroundColor: '#FAF8F5' }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <p className="text-[#2C1810]/80 text-lg leading-relaxed">
              {t.about.description}
            </p>
          </motion.div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                <Heart className="w-6 h-6 text-[#C8A97E]" />
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                <Globe className="w-6 h-6 text-[#C8A97E]" />
                <h3 className="font-playfair text-2xl font-bold text-white">
          ...
          >
            {teamMembers.map((member, index) => (
              <motion.div
                key={index}
                variants={cardVariants}
                className="bg-white rounded-2xl p-8 shadow-sm border border-[#E8D5B5]/30"
              >
                <div className="w-16 h-16 rounded-full gold-gradient flex items-center justify-center mb-4 mx-auto">
                  <Users className="w-8 h-8 text-[#0C0C14]" />
                </div>
                <h3 className="font-playfair text-lg font-bold text-center mb-2" style={{ color: '#2C1810' }}>
                  {locale === 'es' ? member.nameEs : member.nameEn}
                </h3>
                <p className="text-[#8B8680] text-sm leading-relaxed text-center">
