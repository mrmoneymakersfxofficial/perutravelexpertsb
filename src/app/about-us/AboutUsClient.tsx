      ...
      <section className="py-12 md:py-16" style={{ backgroundColor: '#FAF8F5' }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center">
            <p className="text-[#2C1810]/80 text-lg leading-relaxed">{t.about.description}</p>
          </motion.div>
        </div>
      </section>
            ...
            {highlights.map((item, index) => {
              ...
              return (
                <motion.div key={index} variants={cardVariants} whileHover={{ y: -8, transition: { duration: 0.3 } }} className="group bg-white rounded-2xl p-8 shadow-sm hover:shadow-xl transition-shadow duration-300 border border-[#E8D5B5]/30">
                  <div className="w-14 h-14 rounded-xl gold-gradient flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300"><Icon className="w-7 h-7 text-[#0C0C14]" /></div>
                  <h3 className="font-playfair text-xl font-bold mb-3" style={{ color: '#2C1810' }}>{t.about[item.titleKey]}</h3>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="glass-card rounded-2xl p-8">
              <div className="flex items-center gap-3 mb-4"><Heart className="w-6 h-6 text-[#C8A97E]" /><h3 className="font-playfair text-2xl font-bold text-white">{locale === 'es' ? 'Nuestra Misión' : 'Our Mission'}</h3></div>
              <p className="text-white/70 leading-relaxed">{t.about.mission}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="glass-card rounded-2xl p-8">
              <div className="flex items-center gap-3 mb-4"><Globe className="w-6 h-6 text-[#C8A97E]" /><h3 className="font-playfair text-2xl font-bold text-white">{locale === 'es' ? 'Nuestra Visión' : 'Our Vision'}</h3></div>
      ...
      <section className="py-16 md:py-20" style={{ backgroundColor: '#FAF8F5' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="font-playfair text-3xl md:text-4xl font-bold mb-3" style={{ color: '#2C1810' }}>{locale === 'es' ? 'Nuestro Equipo' : 'Our Team'}</h2>
          ...
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {teamMembers.map((member, index) => (
              <motion.div key={index} variants={cardVariants} className="bg-white rounded-2xl p-8 shadow-sm border border-[#E8D5B5]/30">
                <div className="w-16 h-16 rounded-full gold-gradient flex items-center justify-center mb-4 mx-auto"><Users className="w-8 h-8 text-[#0C0C14]" /></div>
                <h3 className="font-playfair text-lg font-bold text-center mb-2" style={{ color: '#2C1810' }}>{locale === 'es' ? member.nameEs : member.nameEn}</h3>
