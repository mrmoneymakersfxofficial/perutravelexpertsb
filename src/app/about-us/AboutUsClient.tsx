...
export default function AboutUsClient() {
  ...
  const sectionIds = ['about-description', 'about-highlights', 'about-mission', 'about-team'];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#F8F6F2' }}>
      ...
      <section id="about-description" className="py-12 md:py-16" style={{ backgroundColor: '#F8F6F2' }}>
      ...
      <section id="about-highlights" className="py-16 md:py-20" style={{ backgroundColor: '#F8F6F2' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {highlights.map((item, index) => {
              ...
              return (
                <motion.div key={index} variants={cardVariants} whileHover={{ y: -8, transition: { duration: 0.3 } }} className="group rounded-2xl p-8 transition-shadow duration-300 bg-white/[0.02] border border-[#E8CC6A]/[0.04]">
      ...
      <section id="about-mission" className="py-16 md:py-20" style={{ backgroundColor: '#0F0F0F' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="rounded-2xl p-8 border border-white/[0.04]">
              <div className="flex items-center gap-3 mb-4"><Heart className="w-6 h-6 text-[#D4A843]" /><h3 className="font-playfair text-2xl font-bold text-white">{locale === 'es' ? 'Nuestra Misión' : 'Our Mission'}</h3></div>
              <p className="text-white/70 leading-relaxed">{t.about.mission}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="rounded-2xl p-8 border border-white/[0.04]">
              <div className="flex items-center gap-3 mb-4"><Globe className="w-6 h-6 text-[#D4A843]" /><h3 className="font-playfair text-2xl font-bold text-white">{locale === 'es' ? 'Nuestra Visión' : 'Our Vision'}</h3></div>
      ...
      <section id="about-team" className="py-16 md:py-20" style={{ backgroundColor: '#F8F6F2' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {teamMembers.map((member, index) => (
              <motion.div key={index} variants={cardVariants} className="rounded-2xl p-8 bg-white/[0.02] border border-[#E8CC6A]/[0.04]">
