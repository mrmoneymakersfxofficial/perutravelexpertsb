      ...
      <section id="faq-list" className="pb-16 md:pb-20" style={{ backgroundColor: '#F8F6F2' }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Accordion type="single" collapsible className="w-full">
              {questions.map((faq, index) => (
                <motion.div key={index} variants={itemVariants}>
                  <AccordionItem
                    value={`item-${index}`}
                    className="bg-white/[0.02] rounded-xl border border-[#E8CC6A]/[0.04] px-6 data-[state=open]:border-[#D4A843]/30 transition-all"
                  >
                    <AccordionTrigger className="text-left font-playfair text-base font-semibold py-5 hover:no-underline" style={{ color: '#1C1C1C' }}>
                      <div className="flex items-start gap-3 pr-4">
                        <HelpCircle className="w-5 h-5 text-[#D4A843] flex-shrink-0 mt-0.5" />
                        <span>{faq.q}</span>
                      </div>
                    </AccordionTrigger>
          ...
          <motion.div
            ...
            className="mt-16 bg-white/[0.02] rounded-2xl p-8 md:p-10 border border-[#E8CC6A]/[0.04] text-center"
          >
